 ...
 * <ul>
 * <li> `browser:uncompressed` - build uncomprssed browser bundle (`lipcore-*.js`)
 * <li> `browser:compressed` - build compressed browser bundle (`lipcore-*.min.js`)
...
function startGulp(name, opts) {
  ...
  var fullname = name ? 'lipcore-' + name : 'lipcore';
  var files = ['lib/**/*.js'];
  var tests = ['test/**/*.js'];
  var alljs = files.concat(tests);

  var buildPath = './node_modules/lipcore-build/';
  ...
  if (browser) {
    ...
    if (name !== 'lib') {
      browserifyCommand = buildBinPath + 'browserify --require ./index.js:' + fullname + ' --external lipcore-lib -o ' + fullname + '.js';
    } else {
      browserifyCommand = buildBinPath + 'browserify --require ./index.js:lipcore-lib -o lipcore-lib.js';
    }
  ...
  gulp.task('release:push', function(cb) {
    git.push('lipcoin-project', 'master', {
      args: ''
    }, cb);
  });
  ...
  gulp.task('release:push-tag', function(cb) {
    ...
    git.tag(name, 'Release ' + name, function() {
      git.push('lipcoin-project', name, cb);
    });
  });
  ...
  var release = function(importance, cb) {
    ...
    return runsequence(
      ...
      // Run git push lipcoin-project $VERSION
      'release:push-tag',
      // Run npm publish
      'release:publish',
